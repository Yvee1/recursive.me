  ...
  space.add(
    ...
    function (time, ftime) {
      ...
      for (let angle = 0, i = 0; angle <= 2 * Math.PI; angle += gap, i++){
        let x = width/2 + Math.cos(-angle + offset + time/10000) * width * 0.3;
        let y = height/2 + Math.sin(2* (-angle + offset + time/10000)) * width/8;
